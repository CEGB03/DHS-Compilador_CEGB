...
class Escucha(compiladoresListener):
    ...
    def enterIfuncion(self, ctx: compiladoresParser.IfuncionContext):
        ...
        # ===== SOLUCIÓN: Cargar argumentos INMEDIATAMENTE =====
        if ctx.ID():
            nombre_funcion = ctx.ID().getText()
            self.tabla.renombrar_contexto_actual(nombre_funcion)
            
            # Extraer argumentos directamente del contexto actual
            args = []
            if ctx.param():
                args = self.extraer_argumentos_param(ctx.param())
                
            # DEBUG: Imprimir argumentos extraídos
            print(f"DEBUG: Argumentos extraídos para función {nombre_funcion}: {args}")

            # REGISTRAR argumentos como variables al contexto INMEDIATAMENTE
            for tipo, nombre_arg in args:
                if self.tabla.buscar_local(nombre_arg) is None:
                    ...
                    print(f"DEBUG: Argumento {nombre_arg} registrado en enterIfuncion")
                else:
                    print(f"DEBUG: Argumento {nombre_arg} ya existe en el contexto")

    def exitIfuncion(self, ctx: compiladoresParser.IfuncionContext):
        # ===== COMENTADO: La lógica anterior que funcionaba pero tardía =====
        # # 1. Obtener el nombre real de la función y renombrar el contexto
        # nombre_funcion = ctx.ID().getText() if ctx.ID() else "BLOQUE_FUNCION"
        # self.tabla.renombrar_contexto_actual(nombre_funcion)
        
        # # 2. Extraer argumentos de la definición ANTES de procesar la función
        # args = []
        # if ctx.param():
        #     args = self.extraer_argumentos_param(ctx.param())

        # # 3. CORRECCIÓN PRINCIPAL: Agregar cada argumento como variable al contexto actual
        # for tipo, nombre_arg in args:
        #     if self.tabla.buscar_local(nombre_arg) is None:
        #         var_arg = Variable(nombre_arg, tipo, inicializado=True, declarado=True)
        #         self.tabla.add_identificador(var_arg)
        #         self.write_log(f"Argumento agregado: {tipo} {nombre_arg}", indentar=True)
        #         print(f"DEBUG: Argumento {nombre_arg} agregado al contexto {nombre_funcion}")
        
        # 4. Procesar la función en la tabla de símbolos global (MANTENER ESTO)
        nombre_funcion = ctx.ID().getText() if ctx.ID() else "BLOQUE_FUNCION"
        linea = ctx.start.line
        tipo_retorno = ctx.tipo().getText() if ctx.tipo() else ""
        
        # Extraer argumentos para verificación con prototipo
        args = []
        if ctx.param():
            args = self.extraer_argumentos_param(ctx.param())
        ...
        # Eliminar el contexto de la función al final
        self.tabla.del_Contexto()
    ...
    def verificar_variables_en_expresion(self, expresion, linea):
        ...
        for token in tokens:
            ...
            print(f"DEBUG: Buscando variable '{token}' en línea {linea}")
            print(f"DEBUG: Contextos activos: {[ctx.nombreContexto for ctx in self.tabla.contextos]}")
            ...
            if var and not isinstance(var, Funcion):
                var.set_usado()
                print(f"DEBUG: Variable '{token}' encontrada y marcada como usada")
                ...
                print(f"DEBUG: Variable '{token}' NO encontrada")
    ...
    def extraer_argumentos_param(self, param_ctx):
        ...
        if hasattr(ctx, 'p') and ctx.p():
            ...
            nombre = ctx.p().ID().getText()
            args.append((tipo, nombre))
            print(f"DEBUG: Encontrado argumento: {tipo} {nombre}")
        ...
        while ctx is not None and hasattr(ctx, 'param') and ctx.param():
            ...
            if hasattr(ctx, 'p') and ctx.p():
                ...
                nombre = ctx.p().ID().getText()
                args.append((tipo, nombre))
                print(f"DEBUG: Encontrado argumento recursivo: {tipo} {nombre}")
